    ...
    <div className="relative overflow-hidden border-y border-[#5A80FB]/20 bg-[#F0F4FF] py-3">
      ...
      >
        {repeated.map((t, i) => (
          <span
            key={i}
            className="text-[10px] font-semibold tracking-[0.25em] uppercase text-[#4563C2]/60"
          >
            {t}
          </span>
        ))}
...
/* ── Cinematic Background (Optimized: Removed heavy mouse parallax on blurs) ── */
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <div className="absolute inset-0">
        <img
          ...
          style={{
            filter:
              "saturate(0.45) brightness(0.35) sepia(0.3) hue-rotate(180deg)",
          }}
        />
      ...
      {/* Static Light orbs - No longer tracking mouse to save massive GPU calculations */}
...
/* ── Hero Section (Optimized: Removed mouse tracking state & springs) ── */
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 pt-32 pb-20">
        ...
        >
          ...
          <motion.div variants={fade} className="flex items-center justify-center gap-4 mb-10">
            ...
            <span className="text-[11px] font-semibold tracking-[0.3em] uppercase text-[#F0F4FF]/80">
              مدیریت هوشمند مدرسه
            </span>
            <div className="h-[1px] w-20 bg-[#5A80FB]/50" />
          </motion.div>
...
/* ── Story Chapter (Optimized: Replaced heavy useScroll with whileInView) ── */
const StoryChapter = memo(({ chapter, chapterEn, image, title, description, icon: Icon, reverse, accent }) => {
  return (
    <section className="relative min-h-screen w-full flex items-center py-24 overflow-hidden bg-[#F0F4FF]">
      <div
      ...
      >
        <div className={`${reverse ? "md:order-2" : "md:order-1"} space-y-6 text-right`}>
          <div className="flex items-center gap-3 justify-end">
            ...
            <span className="text-[9px] font-black tracking-[0.25em] text-[#2A3D7A] bg-[#F0F4FF] border border-[#5A80FB]/50 px-3 py-1 rounded-full">
              فصل {chapter}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 justify-end">
            {["هوشمند", "سریع", "امن", "دیجیتال"].map((tag) => (
              <span key={tag} className="text-[9px] font-semibold tracking-wider bg-[#F0F4FF] border border-[#5A80FB]/50 text-[#4563C2] px-3 py-1.5 rounded-full uppercase">
                {tag}
              </span>
            ))}
          </div>
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-3xl p-7 flex flex-col justify-between overflow-hidden group cursor-default
                  ${s.wide ? "col-span-2" : ""}
                  ${s.dark ? "bg-[#2A3D7A] text-[#F0F4FF] shadow-[0_8px_40px_rgba(42,61,122,0.4)]" : "bg-white border border-[#5A80FB]/40 shadow-[0_4px_20px_rgba(69,99,194,0.08)]"}`}
              >
                {s.dark && (
...
/* ── Final CTA (Optimized: Removed useScroll scale effect) ── */
const FinalCTA = memo(() => {
  const router = useRouter();
  const handleRegisterClick = useCallback(() => router.push("/login"), [router]);

  return (
    <section className="py-32 bg-[#F0F4FF] overflow-hidden relative">
